...
# If termcolor is not installed, you can install it using:
...
def colored_ascii_art(text, color):
    return colored(pyfiglet.figlet_format(text), color)
...
class RainbowTqdm(tqdm):
    ...
    def update(self, n=1):
        super().update(n)
        self.display_rainbow_bar()

    def display_rainbow_bar(self):
        colors = ['red', 'yellow', 'green', 'cyan', 'blue', 'magenta', 'white']
        rainbow_bar = ''.join([colored('#', colors[i % len(colors)]) for i in range(self.n)])
        bar_length = self.total
        print('\r' + rainbow_bar + ' ' * (bar_length - len(rainbow_bar)), end='')
...
def draw_tarot_cards(question):
    ...
    # Simulate drawing cards with a rainbow progress bar
    with RainbowTqdm(total=100, desc=colored("Consulting the spirits...", 'magenta'), ascii=True) as bar:
        for i in range(100):
            time.sleep(0.01)
            bar.update(1)

    # Display the results with colored ASCII art
    for card in drawn_cards:
        orientation = "upright" if random.choice([True, False]) else "reversed"
        card_art = colored_ascii_art(card, 'cyan')
        cprint(card_art, 'cyan')
        print(f"\033[38;5;214mCard: {card} - {orientation.capitalize()}\033[0m")
        print(f"\033[38;5;214mMeaning: {tarot_cards[card][orientation]}\033[0m\n")
...
def daily_card():
    ...
    # Simulate drawing card with a rainbow progress bar
    with RainbowTqdm(total=100, desc=colored("Consulting the spirits...", 'magenta'), ascii=True) as bar:
        for i in range(100):
            time.sleep(0.01)
            bar.update(1)

    # Determine if the card is upright or reversed randomly
    orientation = "upright" if random.choice([True, False]) else "reversed"

    daily_art = colored_ascii_art("Today's Card", 'yellow')
    card_art = colored_ascii_art(daily_tarot_card, 'cyan')
    cprint(daily_art, 'yellow')
    print(f"\033[38;5;214mToday's card ({today.strftime('%Y-%m-%d')}): {daily_tarot_card} - {orientation.capitalize()}\033[0m")
    print(f"\033[38;5;214mMeaning: {tarot_cards[daily_tarot_card][orientation]}\033[0m\n")
    cprint(card_art, 'cyan')

    twinkling_stars()

def twinkling_stars():
    twinkles = itertools.cycle(['*', '.', 'o', '+', '~', '*', '.', 'o', '+', '~', '*', '.', '*', '.', 'o', '+', '~', '*', '.', 'o', '+', '~', '*', '.'])
    colors = ['yellow', 'cyan', 'magenta', 'white']
    stop_thread = threading.Event()

    def animate():
        while not stop_thread.is_set():
            twinkle_string = ''.join([colored(next(twinkles), random.choice(colors)) for _ in range(24)])
            print(f'\r{twinkle_string}', end='', flush=True)
            time.sleep(0.2)

    animation_thread = threading.Thread(target=animate)
    animation_thread.start()

    time.sleep(3)  # Let the animation run for 3 seconds
    stop_thread.set()
    animation_thread.join()

    # Clear the twinkling stars line
    print('\r' + ' ' * 80, end='\r')

# ASCII Art Introduction
welcome_art = colored_ascii_art("Welcome to the Tarot Reader!", 'green')
cprint(welcome_art, 'green')

# Loop to keep asking for user input
while True:
    twinkling_stars()
    user_input = input(colored("\n\nType your question, or type 'daily' for the daily card (Type 'exit' to end):\n", 'magenta')).strip()

    # Check for disallowed characters - hehe security
    if any(char in user_input for char in ["\\", "/", "%", "'"]):
        cprint("Invalid characters detected. Try again.", 'red')
        continue
